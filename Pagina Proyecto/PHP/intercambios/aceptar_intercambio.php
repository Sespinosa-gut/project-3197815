...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // El dueño del producto_objetivo debería ser quien acepta:
    $sql = "UPDATE intercambios i
            JOIN productos p ON p.id = i.producto_objetivo_id
            SET i.estado = 'aceptado', i.actualizado_en = NOW()
            WHERE i.id = ? AND p.usuario_id = ? AND i.estado = 'pendiente'";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ii", $id, $_SESSION['id']);
    if ($stmt->execute() && $stmt->affected_rows === 1) {
        header("Location: ../gestionar-intercambios.html?aceptado=1");
    } else {
